...
print("USING TRAIN_MODEL FROM:", __file__)
...
def train_transformer(model, verify, epochs, learning_rate, batch_size, X, Y, X_test, Y_test, show_attention,cell_type, device, retrain = False, resume = False):
  ...
  # Only instantiate a scheduler when we actually use it. CyclicLR updates the
  # optimizer learning rate during construction, which makes the logs look like
  # training starts at lr=0 even when the active training loop does not use it.
  ...
  val_batches = math.ceil(val_examples / batch_size)
  ...
  for epoch in range(start_epoch, epochs):
    print(f"Starting epoch {epoch + 1}/{epochs} ({num_of_batches} training batches)")
    model.train()
    running_loss = 0
    running_acc = 0
    running_pre = 0
    ...
    for batch_idx, count in enumerate(range(0, num_of_examples - batch_size + 1, batch_size), start=1):
      # repackage_hidden(hidden)
      ...
      if batch_idx == 1 or batch_idx % 10 == 0 or batch_idx == num_of_batches:
        print(
          f"Epoch {epoch + 1}/{epochs} batch {batch_idx}/{num_of_batches} "
          f"loss {loss.item():.4f}"
        )
    ...
    print(f"Starting validation for epoch {epoch + 1}/{epochs} ({val_batches} batches)")

    with torch.no_grad():
      logits_list = []
      model.eval()
      for val_batch_idx, i in enumerate(range(0, val_examples, batch_size), start=1):
        ...
        if val_batch_idx == 1 or val_batch_idx % 20 == 0 or val_batch_idx == val_batches:
          print(f"Validation batch {val_batch_idx}/{val_batches}")
