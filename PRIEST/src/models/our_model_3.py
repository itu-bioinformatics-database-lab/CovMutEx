...
class TimeSeriesTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        self.convs2 = nn.ModuleList([nn.Conv2d(in_channels = 1, 
                                  out_channels = n_filters, 
                                  kernel_size = (fs, vector_size)) 
                                    for fs in filter_sizes])

        self.convs3 = nn.ModuleList([nn.Conv2d(in_channels = 1, 
                                  out_channels = n_filters, 
                                  kernel_size = (fs, vector_size)) 
                                    for fs in filter_sizes])
    ...
    def forward(self, src: Tensor, tgt: Tensor=None, src_mask: Tensor=None, 
        ...
        line1 = src + attention_output
        line2 = src
        line3 = attention_output
    
        # src = self.attention(src)[0]

        conved1 = [F.relu(conv(line1)).squeeze(3) 
                  for conv in self.convs1]

        pooled1 = [F.max_pool1d(conv, conv.shape[2]).squeeze(2) 
                  for conv in conved1]

        conved2 = [F.relu(conv(line2)).squeeze(3) 
                  for conv in self.convs2]

        pooled2 = [F.max_pool1d(conv, conv.shape[2]).squeeze(2) 
                  for conv in conved2]

        conved3 = [F.relu(conv(line3)).squeeze(3) 
                  for conv in self.convs3]

        pooled3 = [F.max_pool1d(conv, conv.shape[2]).squeeze(2) 
                  for conv in conved3]

        cat1 = self.dropout_final(torch.cat(pooled1, dim = 1))
        cat2 = self.dropout_final(torch.cat(pooled2, dim = 1))
        cat3 = self.dropout_final(torch.cat(pooled3, dim = 1))
