...
class RnnModel(nn.Module):
  ...
  def forward(self, input_seq, hidden_state):
    ...
    dummy_attn_weights = torch.zeros(input_seq.shape[1], input_seq.shape[0], device=self.device)
...
class DaRnnModel(nn.Module):
  ...
  def init_hidden(self, batch_size):
    h_init = torch.zeros(1, batch_size, self.m, device=self.device)
    c_init = torch.zeros(1, batch_size, self.m, device=self.device)
