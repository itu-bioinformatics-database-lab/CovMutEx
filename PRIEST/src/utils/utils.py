...
def read_and_process_to_trigram_vecs(data_files, subtype, sample_size=100, test_split=0.0, squeeze=True, extract_epitopes=False):
  ...
  # print(len(train_strains_by_year))
  # print(len(train_strains_by_year[0]))
  # print(train_strains_by_year[0])
...
def process_years(strains_by_year, data_path, squeeze=True, extract_epitopes=False):
  ...
  print('extract_epitopes = ' + str(extract_epitopes))
  if extract_epitopes:
    ...
    # should return a 3d array contaning trigrams that match epitopes

  # print(trigrams_by_year[0])

  print('squeeze = ' + str(squeeze))
  if squeeze:
  ...
  print(f'{len(trigrams_by_year)}x{len(trigrams_by_year[0])}x{len(trigrams_by_year[0][0])}x3')

  trigram_idxs = build_features.map_trigrams_to_idxs(trigrams_by_year, trigram_to_idx)

  # print(type(trigram_idxs))
  # print(type(trigram_idxs[0]))
  # print(type(trigram_idxs[0][0]))
  # print(type(trigram_idxs[0][0][0]))

  print(f'{len(trigram_idxs)}x{len(trigram_idxs[0])}x{len(trigram_idxs[0][0])}x1')
